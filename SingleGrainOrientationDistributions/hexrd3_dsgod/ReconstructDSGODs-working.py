...
if __name__ == '__main__':
    ...
    for i in range(num_paint_grid_iter):
        ...
        # % ORIENTATION SCORING
        # =============================================================================
        logger.info("using map search with paintGrid iteration %i / %i on %d processes" %(i+1, num_paint_grid_iter, ncpus))
        start = timeit.default_timer()
        ...
        for j in range(num_grains_curr_iter):
            ...
            logger.info("Processing grain %i / %i, %i / %i | %s-" %(j+1, num_grains_curr_iter, grain_counter, num_grains, cfg.analysis_id))
