...
if __name__ == '__main__':
    ...
    if not os.path.isfile(eta_ome_npz_save_fname):
    ...
    for igrain, cur_grain_id in enumerate(grain_mat_ids):
        logger.info("Processing grain %i / %i | %s-" %(igrain+1, num_grains, cfg.analysis_id))
        
        # location and name of npz file output    
        dsgod_npz_save_string = dsgod_npz_save_fname %(cur_grain_id)
        ...
        box_shape = ori_Xs.shape
        
        all_exp_maps = ori_grid + cur_exp_maps
        num_oris = all_exp_maps.shape[0]
        all_quats = rotations.quatOfExpMap(all_exp_maps.T)
        
        
        logger.info("will test %d quaternions using %d processes" % (all_quats.shape[1], ncpus))
        ...
        if truncate_comp_thresh is not None:
            ...
            sort_ind = np.argsort(-inten_list, axis=1)
            sort_dsgod_box_inten_list = np.take_along_axis(inten_list, sort_ind, axis=1)
            sort_dsgod_box_filter_list = np.take_along_axis(filter_list, sort_ind, axis=1)
            ...
            # r = np.arange(sort_dsgod_box_inten_list.shape[1])
            # start_mask = (r >= np.asarray(comp_filter_ind)[:,None])
            # end_mask = (r < np.asarray(end_comp_filter_ind)[:,None]) 
            # print(start_mask.shape, end_mask.shape)
            # print(np.sum(start_mask[0, :]), np.sum(end_mask[0, :]))
            # dsgod_box_inten_list[end_mask] = sort_dsgod_box_inten_list[start_mask]
            # dsgod_box_filter_list[end_mask] = sort_dsgod_box_filter_list[start_mask]
            # print(dsgod_box_inten_list.shape, dsgod_box_filter_list.shape)
            
        # re-type to save on space
        dsgod_box_quat = all_quats.astype(np.float32)
        dsgod_box_exp_map = all_exp_maps.astype(np.float32)
        dsgod_box_comp = completeness.astype(np.float16)
        ...
        np.savez(dsgod_npz_save_string, 
                 ...
                 #dsgod_box_exp_map=dsgod_box_exp_map,
