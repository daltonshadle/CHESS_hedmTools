...
# instrument flags (only chi oscillation)
...
# panel [tilt_x, _y, _z, tvecd_x, _y, _z]
panel_flags_DFLT = np.ones(6, dtype=bool)
# dexela needs fixed tvecd_y for the fitting, wild things happen otherwise :)
panel_flags_fixed_Y = np.array([1, 1, 1, 1, 0, 1], dtype=bool)

# distortion
#GE_41RT
GE_distortion_flags_DFLT = np.array([1, 1, 1, 0, 0, 0], dtype=bool)
# Dexela_2923
dexela_distortion_flags_DFLT = np.array([0, 0, 1, 1, 1, 1, 1, 1], dtype=bool)

# grains [expmap_x, _y, _z, tvecc_x, _y, _z, invV_00, _11, _22, _12, _02, _01]
...
# %% *MAIN CALL*
# =============================================================================
def calibrate_instrument_from_rotation_series_sx(cfg_filename, block_id=0, output_name=None,
    ...
    block_id : int, optional
        config block num, only change this if you know what you are doing!. The default is 0.
    output_name : string, optional
        path to save new detector file to. The default is None and will overwrite.
    grain_ids : list, optional
        list of grain ids to calibrate on. The default is [0, 1, 2].
    clobber_strain : bool, optional
        True = reset strains to default (strain free). The default is True.
    clobber_centroid : bool, optional
        True = reset grain centroids to default, only if grain is centerd. 
        The default is False.
    clobber_grain_Y : bool, optional
        True = if using line beam and y position is known. The default is False.
    dexela_fixed_y : bool, optional
        True = using dexela detectors and need to fix y position for fitting.
        The default is True.
    distortion_flags : numpy array, optional
        array of distortion flags for a panel detector, can use defaults above
        in constants. The default is None.
    do_init_plots : bool, optional
        True = do initial plotting. The default is False.

    Returns
    -------
    None. Saves a calibrated detector file.
    ...
    if dexela_fixed_y:
        ...
        if distortion_flags is None:
            panel_param_flags = np.vstack(
                [np.hstack([panel_flags_fixed_Y]),
                  np.tile(np.hstack([panel_flags_DFLT]),
                          (instr.num_panels - 1, 1))]
            ).flatten()
        else:
            # use dexela_distortion_flags_DFLT
            panel_param_flags = np.vstack(
                [np.hstack([panel_flags_fixed_Y, distortion_flags]),
                  np.tile(np.hstack([panel_flags_DFLT, distortion_flags]),
                          (instr.num_panels - 1, 1))]
            ).flatten()
    else:
        #     HYDRA EXAMPLE
        if distortion_flags is None:
            panel_param_flags = np.tile(
                np.hstack([panel_flags_DFLT]),
                instr.num_panels
            )
        else:
            # use GE_distortion_flags_DFLT
            panel_param_flags = np.tile(
                np.hstack([panel_flags_DFLT, distortion_flags]),
                instr.num_panels
            )    
...
if __name__ == '__main__':
    ...
    hexrd_index_fit = 'hexrd find-orientations %s --force\nhexrd fit-grains %s --force'
    
    # dual panel calibration
    cfg_name = base_path + 'mruby_calib_nov_2020_config.yml'
    rc = call(hexrd_index_fit %(cfg_name, cfg_name), shell=True)
    calibrate_instrument_from_rotation_series_sx(cfg_name, block_id=0, output_name=None,
                                                     grain_ids=[0, 1, 2], clobber_strain=True,
                                                     clobber_centroid=False, clobber_grain_Y=False,
                                                     dexela_fixed_y=True, distortion_flags=None)
    
    # multipanel calibration
    cfg_name = base_path + 'mruby_calib_nov_2020_config_mpanel.yml'
    rc = call(hexrd_index_fit %(cfg_name, cfg_name), shell=True)
    calibrate_instrument_from_rotation_series_sx(cfg_name, block_id=0, output_name=None,
                                                     grain_ids=[0, 1, 2], clobber_strain=True,
                                                     clobber_centroid=False, clobber_grain_Y=False,
